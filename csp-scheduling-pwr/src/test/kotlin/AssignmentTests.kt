...
class AssignmentTests {
    ...
    @Test fun `byDay groups properly`() {
        val s1 = Subject("A", Lecture)
        val g1 = listOf(
            Group("", MONDAY, "11:00-12:00", Even)
        )
        val s2 = Subject("B", Lecture)
        val g2 = listOf(
            Group("", MONDAY, "11:00-12:00", Odd)
        )
        val s3 = Subject("C", Lecture)
        val g3 = listOf(
            Group("", TUESDAY, "11:00-12:00", Both)
        )

        val assignment = emptyAssignment<Subject, Group>().apply {
            this[s1] = Domain.of(g1)
            this[s2] = Domain.of(g2)
            this[s3] = Domain.of(g3)
        }

        val actual = assignment.perDay { it.value }
        val expected = mapOf(
